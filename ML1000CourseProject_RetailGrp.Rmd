...
barplot(table(TSdata$grp_level))
...
#Removing the word priority so we can use it as Number
TSdata$priority <-  as.numeric(gsub("Priority ", "", TSdata$priority))
#Removing the word level so we can use it as number
...
TSdata$MTTR <- TSdata$resolved_num - TSdata$close_num

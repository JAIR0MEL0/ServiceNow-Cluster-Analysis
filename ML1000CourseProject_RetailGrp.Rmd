...
rfImp <- randomForest(performance ~ priorityN + levelN + app_category + region + impactN + prod_line, data = sdata, ntree = 100, importance = TRUE)
...
set.seed(152)
trctl <- trainControl(method = 'cv', number = 20, savePredictions = TRUE)
metric <- "Accuracy"
fit.lda <- train(performance ~ ., data=trainDF, method="lda", metric=metric, trControl=trctl)
fit.cart <- train(performance ~ ., data=trainDF, method="rpart", metric=metric, trControl=trctl)
...
```
The Balance accuracy is less than 50% for all performance
           Excellent   Class           Average       Failed
             0.48542   0.4981222       0.498517      0.498972
...
```
The Balance accuracy is better with Random forest but still less than 50% for all performance
           Excellent   Class           Average       Failed
             0.48542   0.4981222       0.498517      0.498972
             0.47812    0.497449      0.4996461      0.499147

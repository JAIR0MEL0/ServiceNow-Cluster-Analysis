...
## Objective
For our analysis, we will predict what performance a ticket will have given an basic data related to the issue.
...
xnew = sdata[5000,c("priorityN","app_category","levelN","region","impactN","prod_line")]
predictions <- predict(fit.rf,xnew)
xnew$priorityN
head(predictions)
saveRDS(fit.cart,'jjts_cart_model')
...
## Objective
For our analysis, we will predict which group a ticket will assigned based on the basic information data related to the issue.

## What's the Problem
Every year, negotiations with vendors focus on the number of members required to provide support activities.  One major indicator is the number of tickets for each Level.  That will give us not only a range of negotiation but also might uncover opportunities to move activities from L3 to L2 and from L2 to L1.

##Feature selection
Let's run a random forest to quantify the relative importance of these features.  We will use features with less than 50 Levels, so Application and Support Group will go away.  Also Dates should not be considered.  Ndays is not a predictable variable because we can't actually trying to Predict the Performance of resolving a ticket when Duration is provided since we won't know how long the ticket will last unresolved, but we will know what Priority the ticket is raised.

Note:
Resolution Category can only be known when the ticket is resolved; so it won't work as predictor.

Summary:
app_category + prod_line + priorityN + region + performance  + impactN

```{r checkingPredictorsLevel, echo=FALSE}
set.seed(719)
sdata$grp_level <- factor(sdata$grp_level)

rfImp <- randomForest(grp_level ~ app_category + prod_line + priorityN + region + performance  + impactN, data = sdata, ntree = 100, importance = TRUE)
importance(rfImp,type=2)
```
...
TSdata <- sdata[,c("grp_level" , "app_category" , "prod_line" , "priorityN", "region", "performance" , "impactN")]
...
```
The Balance accuracy is less than 82.9% for all performance and with the Balanced accuracy:
           Level 1       Level 2        Level 3
           0.7365         0.6948        0.50000
...
confusionMatrix(factor(predictions, levels = c("Level 1","Level 2","Level 3")),factor(testDF$grp_level, levels = c("Level 1","Level 2","Level 3")))
```
The Balance accuracy for Classification Tree is 84.1% for all performance
           Level 1     Level 2    Level 3
           0.7448      0.7032     0.50000
...
```
The Balance accuracy for Random Forest is 85.9% or accuray
           Level 1        Level 2      Level 3
           0.8054         0.7576       0.529462
Random forest is giving us the best accuracy of all three, altough, they are pretty close.
...
## Deployment
We will run a sample to predict the Group Level and generate the model to be used with our Shiny App 
```{r deploymentlevel, echo=FALSE}
xnew = sdata[5000,c("app_category" , "prod_line" , "priorityN", "region", "performance" , "impactN")]
predictions <- predict(fit.rf,xnew)
xnew$grp_level
head(predictions)
saveRDS(fit.cart,'jjts_level_model')
```


##Conclusion:
Accuracy to determine the Level is higher than accuracy to determine the 
